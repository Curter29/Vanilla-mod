...
<div class="Tabs ProfileTabs">
   ...
   foreach ($SortOrder as $TabCode) {
      ...
      if (array_key_exists($TabCode, $this->_ProfileTabs)) {
         ...
         echo '<li'.($CssClass == '' ? '' : ' class="'.$CssClass.'"').'>'.Anchor(GetValue('TabHtml', $TabInfo, $TabCode), GetValue('TabUrl', $TabInfo)).($TabInfo['TabCount'] ? '<span class="sup">'.$TabInfo['TabCount'].'</span>' : '')."</li>\r\n";
      }
   }
   ?>
   </ul>
