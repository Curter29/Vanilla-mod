   ...
   foreach ($this->CategoryData->Result() as $Category) {
      ...
      if ($Category->CategoryID > 0) {
         ...
         if ($Category->Depth < $MaxDisplayDepth && $ChildCategories != '') {
            $CatList = str_replace('{ChildCategories}', '<span class="ChildCategories head_cat">'.Wrap(T('Child Categories:'), 'b').' '.$ChildCategories.'</span>', $CatList);
            $ChildCategories = '';
         }
         ...
         if ($Category->Depth >= $MaxDisplayDepth && $MaxDisplayDepth > 0) {
            ...
            $CatList .= '<li class="Item Depth'.$Category->Depth.' Category-'.$Category->UrlCode.'">
               <div class="ItemContent Category '.$CssClasses.'"><div class="head_cat">'
                  ...
                  .($Category->CountDiscussions ? '<span class="DiscussionCount">'.$Category->CountDiscussions.' '.Gdn_Plural($Category->CountAllDiscussions, 'тема', 'темы', 'тем').'</span>' : '')                  
                  .($Category->Description ? '<div class="Excerpt">'.$Category->Description.'</div>' : '')
                  .'</div>'
                  .'<div class="Meta">'
                    .($Category->LastCommentID != '' && $Category->LastDiscussionName != '' ?
                        '<span class="label pub_info">последняя тема: '
