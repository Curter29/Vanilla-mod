...
<div class="Tabs DiscussionsTabs">
   <ul>
      ...
      <li class="<?=$all_active?> First"><?=Anchor('Все', 'discussions')?></li>
   ...
   if($Sender->CanEditDiscussions){?><span class="AdminCheck ToggleCheck"><label><input type="checkbox" name="Toggle" /><span>отметить все темы</span></label></span><?}?>
</div>
<?
}
...
function WriteDiscussion($Discussion, &$Sender, &$Session, $Alt2) {
   ...
   if ($DiscussionName == '') $DiscussionName = T('Blank Discussion Topic');
      
   $Sender->EventArguments['DiscussionName'] = &$DiscussionName;

   static $FirstDiscussion = TRUE;
   if(!$FirstDiscussion) $Sender->FireEvent('BetweenDiscussion');
   else $FirstDiscussion = FALSE;   
...
<li class="<?=$CssClass?>">
   ...
   <div class="ItemContent Discussion">
      ...
      <div class="Meta">
         ...
         <?if($Discussion->Announce == '1'){?><span class="label anonce"><i></i>важно</span><?}?>
         <?if($Discussion->Closed == '1'){?><span class="label closed"><i></i>закрыта</span><?}?>
         <?
            $Sender->FireEvent('AfterCountMeta');
            $date_class = '';
            $date_str = Gdn_DateF($Discussion->FirstDate, $date_class);
         ?>
         <span class="label pub_info<?=$date_class?>"><?printf('опубликованно %s, %s в: %s', $date_str, UserAnchor($First), Anchor($Discussion->Category, '/categories/'.$Discussion->CategoryUrlCode, 'Category'))?></span>
         <?if($Discussion->CountComments > 1){?><span class="label comments_info"><?printf(Gdn_Plural($Discussion->CountComments, '%s коментарий', '%s коментария', '%s коментариев'), $Discussion->CountComments)?></span><?}?>
         <?if($login){
             if($Discussion->CountComments > 1 && $Discussion->CountUnreadComments){?><span class="label comments_new"><?=$Discussion->CountUnreadComments.' '.Gdn_Plural($Discussion->CountUnreadComments, 'новый', 'новых', 'новых')?></span><?}?>
             <a href="/vanilla/discussion/bookmark/<?=$Discussion->DiscussionID.'/'.$Session->TransientKey().'?Target='.urlencode($Sender->SelfUrl)?>" class="label <?=$Discussion->Bookmarked == '1' ? ' bookmarked' : ' bookmark'?>"><i></i><?=$Discussion->Bookmarked ? 'в избранном' : 'следить за темой'?></a>
         <?}?>
