...
<div class="MessageForm CommentForm">
   <?php if (!$Editing) {
   echo '<h1>Написать комментарий</h1>';
   /*?>
   ...
   */
   ...
   $CancelClass = 'Cancel';
   if (!$NewOrDraft)  $CancelText = 'Cancel';
   ...
   */
   ...
   if(!$Editing)
   {
      ...
      if ($NewOrDraft) echo $this->Form->Button('Save Draft', array('class' => 'Button DraftButton'));
   }
   ...
   if(!$Editing)
   {
      echo '<div class="ButtonsPreview" style="display:none;">';
      echo $this->Form->Button('Post Comment', $ButtonOptions);
      echo $this->Form->Button('&larr; Backt To Edit', array('class' => 'Button BacktToEdit')); 
      echo '</div>';
   }
