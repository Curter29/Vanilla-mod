...
function WriteComment($Object, $Sender, $Session, $CurrentOffset) {
   ...
   if ($Type == 'Comment') {
      ...
      if ($Permalink === FALSE) $Permalink = '/discussion/comment/'.$Object->CommentID.'/#Comment_'.$Object->CommentID;
   } else {
      $Sender->EventArguments['Discussion'] = $Object;   
      $CssClass .= ' FirstComment';
      $Id = 'Discussion_'.$Object->DiscussionID;
      if($Permalink === FALSE) $Permalink = '/discussion/'.$Object->DiscussionID.'/'.Gdn_Format::Url($Object->Name).'/p1';
   }
   ...
   //$comment_class = '';
   //$comment_date = Gdn_DateF($Object->DateInserted, $comment_class);
   //$avatar = UserPhoto($Author);
   //$avatar = $avatar ? $avatar : '<a href="/profile/'.$Author->UserID.'/'.rawurlencode($Author->Name).'"><img src="/uploads/stub-avatar.gif"></a>';

   $admin_check = $toggle_check = '';
   if($Sender->CanEditComments){
      if ($Sender->EventArguments['Type'] == 'Comment') {
         $_Id = $Object->CommentID;
         if(!property_exists($Sender, 'CheckedComments')) $Sender->CheckedComments = $Session->GetAttribute('CheckedComments', array());
         $ItemSelected = InSubArray($_Id, $Sender->CheckedComments);
         $admin_check = '<span class="AdminCheck"><input type="checkbox" name="CommentID[]" value="'.$_Id.'"'.($ItemSelected?' checked="checked"':'').' /></span>';
      } else {
         $admin_check = '<span class="AdminCheck ToggleCheck"><label><input type="checkbox" name="Toggle" /><span>отметить все сообщения</span></label></span>';
         $toggle_check = 1;
      }
   }   
?>
<li class="<?=$CssClass?>" id="<?=$Id?>">
   <a name="Item_<?=$CurrentOffset+1?>" rel="nofollow"></a>
   <?if($toggle_check && $admin_check) echo $admin_check;?>
   <div class="Comment">    
    <?=Gdn_UserBox($Author, $Object->DateInserted, $toggle_check ? '' : $admin_check)?>
    ...
    <div class="MessageOptions"><?WriteOptionList($Object, $Sender, $Session);?></div>
...
function WriteOptionList($Object, $Sender, $Session) {
   ...
   if ($Sender->EventArguments['Type'] == 'Discussion') {
      ...
      if (($CanEdit && $Session->UserID == $Object->InsertUserID) || $Session->CheckPermission('Vanilla.Discussions.Edit', TRUE, 'Category', $PermissionCategoryID))
         $Sender->Options .= '<span>'.Anchor('Редактировать', '/vanilla/post/editdiscussion/'.$Object->DiscussionID, 'EditDiscussion').$TimeLeft.'</span>';
         
      /*// Can the user announce?
      if ($Session->CheckPermission('Vanilla.Discussions.Announce', TRUE, 'Category', $PermissionCategoryID))
         $Sender->Options .= '<span>'.Anchor(T($Sender->Discussion->Announce == '1' ? 'Снять пометку "важно"' : 'Пометить важной'), 'vanilla/discussion/announce/'.$Object->DiscussionID.'/'.$Session->TransientKey(), 'AnnounceDiscussion') . '</span>';
      ...
      if ($Session->CheckPermission('Vanilla.Discussions.Close', TRUE, 'Category', $PermissionCategoryID))
         $Sender->Options .= '<span>'.Anchor($Sender->Discussion->Closed == '1' ? 'Открыть' : 'Закрыть', 'vanilla/discussion/close/'.$Object->DiscussionID.'/'.$Session->TransientKey().'?Target='.urlencode($Sender->SelfUrl), 'CloseDiscussion') . '</span>';
      */
      
      // Can the user delete?
      if ($Session->CheckPermission('Vanilla.Discussions.Delete', TRUE, 'Category', $PermissionCategoryID))
         $Sender->Options .= '<span>'.Anchor('Удалить', 'vanilla/discussion/delete/'.$Object->DiscussionID.'/'.$Session->TransientKey(), 'DeleteDiscussion') . '</span>';
   } else {
      ...
      if (($CanEdit && $Session->UserID == $Object->InsertUserID) || $Session->CheckPermission('Vanilla.Comments.Edit', TRUE, 'Category', $PermissionCategoryID))
         $Sender->Options .= '<span>'.Anchor('Редактировать', '/vanilla/post/editcomment/'.$Object->CommentID, 'EditComment').$TimeLeft.'</span>';

      // Can the user delete the comment?
      if ($Session->CheckPermission('Vanilla.Comments.Delete', TRUE, 'Category', $PermissionCategoryID))
         $Sender->Options .= '<span>'.Anchor('Удалить', 'vanilla/discussion/deletecomment/'.$Object->CommentID.'/'.$Session->TransientKey().'/?Target='.urlencode("/discussion/{$Object->DiscussionID}/x"), 'DeleteComment') . '</span>';
   }
